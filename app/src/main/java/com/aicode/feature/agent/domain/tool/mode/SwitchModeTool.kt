...
class SwitchModeTool @Inject constructor(
    ...
    override val description = "切换当前会话模式（PLAN/BUILD），需用户授权。BUILD 下遇到复杂任务可申请进入 PLAN；PLAN 计划完成后可申请切到 BUILD。AUTO 模式只能由用户在界面手动进入；处于 AUTO 时可用本工具切到 PLAN 退出（唯一路径），不能切到 BUILD。"

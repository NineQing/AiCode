...
浏览器内置夜间模式，默认跟随 App 外观：App 为深色时页面自动反色变暗，App 为浅色时恢复原样。底栏的月亮/太阳图标可手动切换；手动选择后不再跟随 App 外观，切回与 App 外观一致时会自动恢复跟随。

夜间模式通过反色渲染实现：文字与背景反转，图片和视频会二次反色还原原始观感，因此照片颜色正常。少数固定定位元素可能显示轻微异常。该效果作用于页面渲染，AI 截图同样会呈现夜间配色。

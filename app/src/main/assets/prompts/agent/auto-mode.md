...
- 你无法通过 `planMode` 工具进入 AUTO 模式，该模式只能由用户在界面上手动进入。但如果你需要离开 AUTO 模式，可用 `planMode(action="enter")` 进入 PLAN 模式（这是 AI 退出 AUTO 的唯一路径）。进入 PLAN 是临时的：计划完成、用户批准退出后，会自动恢复到 AUTO 模式，无需你手动切回。

...
费用是**估算值**，优先用你在「提供商 → 模型」里填的自定义单价，没填的才按 models.dev 的公开单价算：缓存读取按缓存价算，没有缓存价时按输入价的 10% 估；缓存写入按缓存写入价算，没有时按输入价的 1.25 倍估。总费用按「渠道 + 模型」分别套单价再求和，所以同一个模型从不同提供商调用会各自计价。缓存命中率是缓存输入除以总输入。
...
**渠道统计**按提供商聚合，每行显示渠道名、调用次数、输入输出消耗和占比进度条，按总消耗倒序。

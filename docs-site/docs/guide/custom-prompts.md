...
::: tip v1.12.0 起
本文的片段命名规则（`<两位数字>-<名称>.md`）、按数字新增片段、`.no-builtin` 完全禁用开关与 `{{AICODE_*}}` 变量自 **v1.12.0** 起支持。更早的版本只能放与默认片段**完全同名**的文件来覆盖，不支持按数字新增、`.no-builtin` 与变量。
:::
...
其它占位符（如压缩提示词用的 `{{INSTRUCTION}}`）不受影响。
